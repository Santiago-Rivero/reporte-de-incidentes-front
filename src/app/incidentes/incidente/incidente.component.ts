...
export class IncidenteComponent implements OnInit {
  ...
  buscarServiciosDelCliente() {
    ...
    const razonSocial = this.formulario.controls['razon_social'].value;
    const cuit = this.formulario.controls['cuit'].value;
    this.api.clienteSegun(razonSocial, cuit).subscribe((c: any) => {
      this.cliente = c[0];
